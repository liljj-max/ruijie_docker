...
- 已完成：任务解析、9 类检测、同帧 ArUco 货位关联、增量货架扫描、库存预留/消费、固定区闭环原语，以及随机障碍走廊双向 A* + DWB。
- 安全约束：任务 ID 只作订单身份，不用于推导货位；LaserScan/odom/TF 过期或不对时立即停车；机械臂收回后才移动底盘。
- 进行中：完整随机五订单仿真回归、ArUco 可见率和抓取到位参数验证。
- 详见 `智慧零售_下一步执行规划.md`。
